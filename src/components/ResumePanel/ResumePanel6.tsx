// src/components/ResumePanel/ResumePanel6.tsx - Corporate Green Style
...
interface ResumePanel6Props {
  userBasicInfo: any;
}

export const ResumePanel6: React.FC<ResumePanel6Props> = ({ userBasicInfo }) => {
  ...
  return (
    <div id="resume-pane" className="h-full flex flex-col resume6-print bg-white">
      <style>{`
        @media print {
          @page {
            size: A4;
      ...
      `}</style>
      {/* Print Button */}
      <div className="p-4 border-b border-green-200 flex justify-end flex-shrink-0">
        <button
          onClick={() => printResume(resume)}
          className="rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2 text-xs font-medium text-white shadow hover:from-emerald-700 hover:to-teal-700 focus:outline-none focus:ring-2 focus:ring-emerald-300 transition-all"
        >
          הדפס \ ייצא PDF
        </button>
      </div>
      ...
      {/* Resume Content */}
      <div className="flex-1 overflow-y-auto p-8" style={{ scrollbarWidth: 'thin', scrollbarColor: '#10b981 #f0fdf4' }}>
        {/* Header Section - Corporate Style */}
        <div className="bg-white border-t-8 border-emerald-500 rounded-lg shadow-xl p-8 mb-8">
          <div className="flex items-center gap-8">
            {/* Profile Image Placeholder */}
            <div className="flex-shrink-0">
              <div className="w-32 h-32 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg flex items-center justify-center shadow-lg">
            ...
            <div className="flex-1">
              ...
              {resume.title?.trim() && (
                <p className="text-2xl text-emerald-600 font-semibold mb-4">{resume.title}</p>
              )}
