// src/components/ResumePanel/ResumePanel4.tsx - Modern Pink Style
...
interface ResumePanel4Props {
  userBasicInfo: any;
}

export const ResumePanel4: React.FC<ResumePanel4Props> = ({ userBasicInfo }) => {
  ...
  return (
    <div id="resume-pane" className="h-full flex flex-col resume4-print bg-gradient-to-br from-pink-50 to-rose-50">
      <style>{`
        @media print {
          @page {
            size: A4;
      ...
      {/* Print Button */}
      <div className="p-4 border-b border-pink-200 flex justify-end flex-shrink-0">
        <button
          onClick={() => printResume(resume)}
          className="rounded-xl bg-gradient-to-r from-pink-600 to-rose-600 px-4 py-2 text-xs font-medium text-white shadow-lg hover:from-pink-700 hover:to-rose-700 focus:outline-none focus:ring-2 focus:ring-pink-300 transition-all transform hover:scale-105"
        >
          הדפס \ ייצא PDF
        </button>
      </div>
      ...
      {/* Resume Content */}
      <div className="flex-1 overflow-y-auto p-6" style={{ scrollbarWidth: 'thin', scrollbarColor: '#ec4899 #fce7f3' }}>
        {/* Header Section - Modern Pink */}
            ...
            {displayExperiences.map((exp: any, index: number) => {
              ...
              return (
                <div key={exp.id || exp.company} className="relative">
                  <div className="absolute right-0 top-2 w-3 h-3 bg-gradient-to-br from-pink-500 to-rose-600 rounded-full"></div>
                  {index < displayExperiences.length - 1 && (
                    <div className="absolute right-1.5 top-5 w-0.5 h-full bg-gradient-to-b from-pink-200 to-transparent"></div>
                  )}

                  <div className="mr-8 bg-gradient-to-r from-pink-50 to-rose-50 rounded-xl p-4 border border-pink-100">
                    <h4 className="font-bold text-gray-900 text-lg">{exp.title}</h4>
                    <p className="text-pink-600 font-medium mb-3">{exp.company} • {displayDuration}</p>
              ...
              return (
                <div key={edu.id || edu.institution} className="relative">
                  ...
                  <div className="mr-8 bg-gradient-to-r from-pink-50 to-rose-50 rounded-xl p-4 border border-pink-100">
                    ...
                    <p className="text-pink-600 font-medium mb-3">{edu.institution} • {displayDuration}</p>
            ...
            {allSkills.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {allSkills.map((skill, index) => (
                  <div
                    key={index}
                    className="bg-gradient-to-r from-pink-100 to-rose-100 text-pink-800 text-sm font-semibold px-4 py-3 rounded-xl border border-pink-200 text-center hover:from-pink-200 hover:to-rose-200 transition-all"
                  >
                    {skill}
                  </div>
                ))}
