...
/**
 * Validate action before applying it
 */
export function validateAction(action: any): ValidationError[] {
  ...
  if (!action.action) {
    errors.push({
      ...
      message: 'Action missing required "action" field',
      path: 'action',
      severity: 'error'
    });
    return errors;
  ...
  switch (action.action) {
    case 'appendChild':
      ...
      if (!action.node) {
        errors.push({
          type: 'missing_content',
          message: 'appendChild action missing required "node" field',
          path: 'action.node',
          severity: 'error'
        });
      }
    ...
    case 'insertSibling':
      if (!action.after) {
        errors.push({
          type: 'invalid_address',
          message: 'insertSibling action missing required "after" field',
          path: 'action.after',
          severity: 'error'
        });
      }
      if (!action.node) {
        errors.push({
          type: 'missing_content',
          message: 'insertSibling action missing required "node" field',
          path: 'action.node',
          severity: 'error'
        });
      }
    ...
    case 'replaceText':
      if (!action.id) {
        errors.push({
          type: 'invalid_address',
          message: 'replaceText action missing required "id" field',
          path: 'action.id',
          severity: 'error'
        });
      }
      if (!action.text) {
        errors.push({
          type: 'missing_content',
          message: 'replaceText action missing required "text" field',
          path: 'action.text',
          severity: 'error'
        });
      }
    ...
    case 'update':
      if (!action.id) {
        errors.push({
          type: 'invalid_address',
          message: 'update action missing required "id" field',
          path: 'action.id',
          severity: 'error'
        });
      }
      if (!action.patch) {
        errors.push({
          type: 'missing_content',
          message: 'update action missing required "patch" field',
          path: 'action.patch',
          severity: 'error'
        });
      }
    ...
    case 'move':
      if (!action.id) {
        errors.push({
          type: 'invalid_address',
          message: 'move action missing required "id" field',
          path: 'action.id',
          severity: 'error'
        });
      }
    ...
    case 'remove':
      if (!action.id) {
        errors.push({
          type: 'invalid_address',
          message: 'remove action missing required "id" field',
          path: 'action.id',
          severity: 'error'
        });
      }
    ...
    case 'reorder':
      ...
      if (!action.order || !Array.isArray(action.order)) {
        errors.push({
          type: 'invalid_address',
          message: 'reorder action missing required "order" array field',
          path: 'action.order',
          severity: 'error'
        });
      }
